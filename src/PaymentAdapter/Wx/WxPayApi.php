<?php
namespace Fenghuohuo\Payment\PaymentAdapter\Wx;

use Fenghuohuo\Payment\PaymentAdapter\Wx\WxPayData;
/**
 * 
...
class WxPayApi
{
	...
	public function unifiedOrder($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsNotify_urlSet()){
			$inputObj->SetNotify_url($this->option['WX_NOTIFY_URL']);//异步通知url
		}
		
		$inputObj->SetAppid($this->option['WX_APPID']);//公众账号ID
		$inputObj->SetMch_id($this->option['WX_MCHID']);//商户号
		$inputObj->SetSpbill_create_ip($_SERVER['REMOTE_ADDR']);//终端ip	  
		//$inputObj->SetSpbill_create_ip("1.1.1.1");
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
		
		//签名
		$inputObj->SetSign($this->option['WX_KEY']);
		$xml = $inputObj->ToXml();
		
		$startTimeStamp = $this->getMillisecond();//请求开始时间
		$response = $this->postXmlCurl($xml, $url, false, $timeOut);
		$result = WxPayResults::Init($response, $this->option['WX_KEY']);
	...
	public function orderQuery($inputObj, $timeOut = 6)
	{
		...
		$inputObj->SetSign($this->option['WX_KEY']);//签名
		$xml = $inputObj->ToXml();
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public function sendredpack($inputObj, $timeOut = 6)
	{
		$url = "https://api.mch.weixin.qq.com/mmpaymkttransfers/sendredpack";
		//检测必填参数
		if(!$inputObj->IsMch_billnoSet()) {
			throw new \Exception("商户订单号，缺少必填参数Mch_billno！", 10);
		} if(!$inputObj->IsSend_nameSet()) {
			throw new \Exception("商户名称，缺少必填参数send_name！", 10);
		} if(!$inputObj->IsRe_openidSet()) {
			throw new \Exception("用户openid，缺少必填参数re_openid！", 10);
		} if(!$inputObj->IsTotal_amountSet()) {
			throw new \Exception("付款金额，缺少必填参数total_amount！", 10);
		} if(!$inputObj->IsTotal_numSet()) {
			throw new \Exception("红包发放总人数，缺少必填参数total_num！", 10);
		} if(!$inputObj->IsWishingSet()) {
			throw new \Exception("红包祝福语，缺少必填参数wishing！", 10);
		} if(!$inputObj->IsClient_ipSet()) {
			throw new \Exception("Ip地址，缺少必填参数client_ip！", 10);
		} if(!$inputObj->IsAct_nameSet()) {
			throw new \Exception("活动名称，缺少必填参数act_name！", 10);
		} if(!$inputObj->IsRemarkSet()) {
			throw new \Exception("备注，缺少必填参数remark！", 10);
		}
		$inputObj->SetWxappid($this->option['WX_APPID']);//公众账号ID
		$inputObj->SetMch_id($this->option['WX_MCHID']);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
	 ...
	 * @return 成功时返回，其他抛异常
	 */
	public function gethbinfo($inputObj, $timeOut = 6)
	{
		...
		if(!$inputObj->IsMch_billnoSet()) {
			...
			throw new \Exception("订单类型，缺少必填参数bill_type！");
		}
		$inputObj->SetAppid($this->option['WX_APPID']);//公众账号ID
		$inputObj->SetMch_id($this->option['WX_MCHID']);//商户号
		$inputObj->SetNonce_str(self::getNonceStr());//随机字符串
 	 ...
 	 */
	public function notify($xml, $callback, &$msg)
	{
		//如果返回成功则验证签名
		try {
			$result = WxPayResults::Init($xml, $this->option['WX_KEY']);
		} catch (\Exception $e){
			$msg = $e->errorMessage();
			return false;
		}
	...
	{		
		...
		if($useCert == true){
			...
			curl_setopt($ch,CURLOPT_SSLCERT, $this->option['WX_PUBLIC_KEY_PATH']);
			curl_setopt($ch,CURLOPT_SSLKEYTYPE,'PEM');
			curl_setopt($ch,CURLOPT_SSLKEY, $this->option['WX_PRIVATE_KEY_PATH']);
		}
		//post提交方式
